defmodule Bigtable.Ecto.Migrator do
  @moduledoc """
  ...
  def migrated_versions(repo, opts \\ []) do
    ...
    get_migrations(repo, opts, fn versions -> versions end)
  end
  ...
  def up(repo, version, module, opts \\ []) do
    ...
    get_migrations(repo, opts, fn versions ->
  ...
  defp do_down(repo, version, module, opts) do
    async_migrate_maybe_in_transaction(repo, version, :down, opts, fn ->
  ...
  defp async_migrate_maybe_in_transaction(repo, version, direction, opts, fun) do
    parent = self()
    ref = make_ref()
    task = Task.async(fn -> run_maybe_in_transaction(parent, ref, fun) end)
  ...
  defp run_maybe_in_transaction(parent, ref, fun) do
